...
def digits_to_payload(digits: list[int], mode: str = "ascii3") -> str:
    ...
    if mode == "ascii3":
        if any(v > 127 for v in values):
            raise ValueError('Rain digits fall outside the historical ASCII domain.')
        return "".join(chr(v) for v in values)

    if any(v > 255 for v in values):
        raise ValueError('Rain digits do not describe UTF-8 bytes.')
...
def encode(carrier: str, payload: str, mode: str = "ascii3", spread: bool = True) -> str:
    if any(ch in MOON_TEAR_MARKS for ch in carrier):
        raise ValueError('Carrier already contains reserved moon-tear marks; choose an unmarked carrier.')
...
def decode(text: str, mode: str = "ascii3") -> str:
    ...
    def flush() -> None:
        ...
        if current_base is not None and is_carrier_char(current_base):
            ...
            if count > 7:
                raise ValueError('Invalid rain stack: more than seven reserved marks.')
